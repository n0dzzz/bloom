...
class __declspec(align(16))VecAligned : public Vector
{
public:
	VecAligned& operator=(const Vector& vOther)
	{
...
struct ray_t
{
	VecAligned m_start; // starting point, centered within the extents
	VecAligned m_delta; // direction + length of the ray
	VecAligned m_start_offset; // Add this to m_Start to get the actual ray start
	VecAligned m_extents; // Describes an axis aligned box extruded along a ray
...
{
public:
	virtual int get_point_contents(const Vector& pos, int mask = MASK_ALL, IHandleEntity** ent = nullptr) = 0;
	...
	virtual void clip_ray_to_entity(const ray_t& ray, unsigned int mask, CBasePlayer* ent, trace_t* trace) = 0;
